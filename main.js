...
const vrcOscSocket = dgram.createSocket('udp4');  // VRChat OSC 字幕专用 socket
...
ipcMain.handle('upload-to-workspace', async (event, { targetDirPath, sourceFilePaths }) => {
  try {
    ...
    // ★ VRChat OSC Chatbox：发送字幕到 VRChat
    ipcMain.handle('vrc-osc-chatbox', (event, { host, port, message }) => {
      try {
        // VRChat Chatbox Input: /chatbox/input [string, bool, bool]
        // arg1: message text, arg2: send immediately (true), arg3: play notification sfx (false)
        const oscMsg = osc.writePacket({
          address: '/chatbox/input',
          args: [
            { type: 's', value: String(message).substring(0, 144) }, // VRC 限制 144 字符
            { type: 'T', value: true },   // send immediately
            { type: 'F', value: false },  // no notification sound
          ],
        });
        vrcOscSocket.send(oscMsg, port, host, (err) => {
          if (err) console.error('[VRC-OSC] send error:', err);
        });
        return { success: true };
      } catch (err) {
        console.error('[VRC-OSC] error:', err);
        return { success: false, error: err.message };
      }
    });

    // VRChat OSC Typing indicator
    ipcMain.handle('vrc-osc-typing', (event, { host, port, typing }) => {
      try {
        const oscMsg = osc.writePacket({
          address: '/chatbox/typing',
          args: [
            typing ? { type: 'T', value: true } : { type: 'F', value: false },
          ],
        });
        vrcOscSocket.send(oscMsg, port, host, (err) => {
          if (err) console.error('[VRC-OSC] typing send error:', err);
        });
        return { success: true };
      } catch (err) {
        return { success: false, error: err.message };
