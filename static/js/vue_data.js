...
let vue_data = {
    ...
    visionSettings: {
      ...
      cameraEnabled: false,
      cameraIndex: 0,
      cameraPrompt: '客观列出画面中的事实：人物（性别/衣着/表情/动作/位置）、场景环境、可读文字（原样抄录）、其他显著细节。只描述事实，不加主观评价。',
      cameraSelectedProvider: null,
      cameraBaseUrl: '',
      cameraApiKey: '',
      cameraModel: '',
      ...
      qwenVlExtraArgs: '--dtype bfloat16 --max-new-tokens 512',
      qwenVlCommandTemplate: '',
      qwenVlAutoFillCamera: true,
    },
    isCameraSnapshotLoading: false,
    captureSources: { monitors: [], windows: [], cameras: [] },
    captureSourcesLoading: false,
    qwenVlStatus: {
      running: false,
      ready: false,
      port_alive: false,
  ...
  "mcpServers": {
    ...
    MoreButtonDict: [
      ...
      {"name": "cameraSnapshotButton", "enabled": true},
    ...
    dockerExamples: `docker pull ailm32442/yui-mhcp001:latest
docker run -d \\
  -p 3456:3456 \\
  -v ./super-agent-data:/app/data \\
  ailm32442/yui-mhcp001:latest
`,
    dockerExamples2: `git clone https://github.com/KiritoYG/yui-mhcp001.git
cd yui-mhcp001
docker-compose up -d
`,
    browserEmbedCodeExamples: `<div id="yui-mhcp001">
  <iframe 
  ...
  <p>Powered By<a href="${backendURL}/chat.html">YUI-MHCP001</a></p>
</div>`,
    codeExamples: {
...
response = client.chat.completions.create(
  model="super-model",
  messages=[
      {"role": "user", "content": "什么是YUI-MHCP001？"}
  ]
)
